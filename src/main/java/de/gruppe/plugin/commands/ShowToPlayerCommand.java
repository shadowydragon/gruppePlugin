...
public class ShowToPlayerCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 1) {
            if (sender instanceof Player player) {
                ...
                if (!player.canSee(Objects.requireNonNull(Bukkit.getPlayer(args[0])))) {
                    player.showPlayer(Main.plugin, Objects.requireNonNull(Bukkit.getPlayer(args[0])));
                }
            }
        }
        if (args.length == 2 && args[0].equalsIgnoreCase("all")) {
            ...
            Util.showPlayersToPlayer(player, onlinePlayers);
        }
        return false;
    }
}
